...
export const addMembers = TryCatch(async (req, res, next) => {
  ...
  if (!chat) return next(new Error("Chat not found", 404));

  if (!chat.groupChat) return next(new Error("This is not a group chat", 400));

  if (chat.creator.toString() !== req.user.toString())
    return next(new Error("You are not allowed to add members", 403));
  ...
  if (chat.members.length > 100)
    return next(new Error("Group members limit reached", 400));
...
export const removeMember = TryCatch(async (req, res, next) => {
  ...
  if (chat.members.length <= 3)
    return next(new Error("Group must have at least 3 members", 400));
...
export const sendAttachments = TryCatch(async (req, res, next) => {
  ...
  if (files.length < 1)
    return next(new Error("Please provide attachments", 400));
...
export const renameGroup = TryCatch(async (req, res, next) => {
  ...
  if (chat.creator.toString() !== req.user.toString())
    return next(new Error("You are not allowed to rename the group", 403));
...
export const deleteChat = TryCatch(async (req, res, next) => {
  ...
  if (chat.groupChat && chat.creator.toString() !== req.user.toString())
    return next(new Error("You are not allowed to delete the group", 403));

  if (!chat.groupChat && !chat.members.includes(req.user.toString())) {
    return next(new Error("You are not allowed to delete the chat", 403));
  }
...
export const getMessages = TryCatch(async (req, res, next) => {
  ...
  if (!chat.members.includes(req.user.toString()))
    return next(new Error("You are not allowed to access this chat", 403));
